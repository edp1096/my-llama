...
void ggml_cuda_mem_free(void *ptr) {
    CUDA_CHECK(cudaFree(ptr));
}
...
void ggml_cuda_destroy() {
    ...
    CUDA_CHECK(cudaStreamDestroy(g_cudaStreams[0]));
    CUDA_CHECK(cudaStreamDestroy(g_cudaStreams2[0]));
    CUDA_CHECK(cudaEventDestroy(g_cudaEvents[0]));

    // CUDA_CHECK(cudaDeviceSynchronize());

    // cublasDestroy(g_cublasH);
}
